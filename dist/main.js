...
import { formatVersion, getLatestIntellijReleaseInfo, parseSemver, } from './jetbrains/versions.ts';
async function checkFileChangeCount() {
    core.debug('BEFORE: simpleGit().status()');
    const statusResult = await simpleGit().status();
...
async function run() {
    ...
    const releaseInfo = await getLatestIntellijReleaseInfo();
    core.debug(`Latest IntelliJ Release Info:`);
    core.debug(JSON.stringify(releaseInfo));
    const latestVersion = parseSemver(releaseInfo.version);
    ...
    if (gradlePropertyVersionName !== 'pluginVersion' && gradlePropertyVersionName !== 'libraryVersion') {
        throw new Error(`Invalid gradlePropertyVersionName: [${gradlePropertyVersionName}] Pick either 'pluginVersion' or 'libraryVersion'.`);
    }
    ...
    const filesChanged = await checkFileChangeCount();
