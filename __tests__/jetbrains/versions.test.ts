...
describe('files.ts', () => {
    ...
    it.each([
        ...
        // -------------------------------
    ])('a %p platform update from %p to %p bumps the plugin version from %p to %p', (releaseType: semver.ReleaseType, currentPlatform: string, newPlatform: string, startingPluginVersion: string, expectedPluginVersion: string) => {
        // given
        const pluginVersion = new semver.SemVer(startingPluginVersion);

        // when
        const nextPluginVersion = files._next_plugin_version(
            pluginVersion,
            currentPlatform,
            newPlatform,
        );
